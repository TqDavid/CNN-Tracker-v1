...
#In this file the region of DISineterst will be selected 
#inside the regions of interest, as soon as the object hits
#a region of disinterest, the object will be counted, it's location 
#data will be saved, the tracking object will be removed, etc.
#The region of disinterest marks the endpoint of tracking
...
def run(im):
    im_disp = im.copy()
    im_draw = im.copy()
    window_name = "Select Region(s) of DISinterest"
    ...
    def callback(event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            ...
            im_draw = im.copy()
            cv2.rectangle(im_draw, pts_1[-1], (x, y), (0,0,255), 2) #RED rectangle for ROD
            cv2.imshow(window_name, im_draw)
    ...
    print "Press, drag and release your mouse to select your Regions of DISinterest"
    ...
    while True:
        ...
        window_name_2 = "Current ROD Selection"
        for pt1, pt2 in zip(pts_1, pts_2):
            rects.append([pt1[0],pt2[0], pt1[1], pt2[1]])
            cv2.rectangle(im_disp, pt1, pt2, (0, 0, 255), 2)
        ...
        if key == ord('p'):
            ...
            if run.mouse_down == False and pts_1:
                print "ROD deleted at  [{}, {}]".format(pts_1[-1], pts_2[-1])
                pts_1.pop()
                pts_2.pop()
                im_disp = im.copy()
            else:
                print "No ROD to delete."
    cv2.destroyAllWindows()
    point= [(tl + br) for tl, br in zip(pts_1, pts_2)]
    corrected_point=check_point(point)
    return corrected_point
...
if __name__ == "__main__":
    ...
    try:
        ...
        exit()
    points = run(im)
    print("Rectangular Regions of Disinterest selected are -> ", points)